...
class EmotionsDetector:
    ...
    self._emotions = OrderedDict([(0, 'neutral'), (1, 'happiness'), (2, 'sadness'), (3, 'anger'), (4, 'fear'),  (5, 'surprise'), (6, 'disgust')])
    modulePath = os.path.dirname(__file__)
    self._modelFile = os.path.abspath('{}/models/emotions_model.dat' \.format(modulePath))
    if not os.path.isfile(self._modelFile):
            raise InvalidModelException('Could not find model file: {}' \.format(self._modelFile))
    if not self.load():
            raise InvalidModelException('Could not load model from file: {}' \.format(self._modelFile))
    def load(self):
        try:
